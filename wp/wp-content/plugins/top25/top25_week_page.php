...
function races_sort($a,$b) {
	return strnatcmp($b['races_mod'], $a['races_mod']);
}
function winning_sort($a,$b) {
	return strnatcmp($b['winning_perc'], $a['winning_perc']);
}
function computer_sort($a,$b) {
	return strnatcmp($b['comp_total'], $a['comp_total']);
}
...
function top25_week() {
	...
	for ($i=0;$i<count($rider_uci_points);$i++) {
		...
		$winning_perc=number_format(($wins/$total_rider_races),3);
	...
	foreach($rider_uci_points as $key=>$rider) {
		...
		foreach ($vote_final as $k=>$v) {
			if ($rider["name"]==$v["name"]) {
				$rider_uci_points[$key]["vote"]=array('pts'=>$v["points"],'perc'=>$v["perc"],'rank'=>$v["rank"]);
	...
	foreach ($rider_uci_points as $k => $rider) {
		$uci_perc=0;
		$uci_perc=number_format(($rider["points"]/$max_points),3);
		$rider_uci_points[$k]["points_perc"]=$uci_perc;
	}
	//print_r($rider_uci_points);
	
	//////////////// Combine SOS and Races Perc for Total Races Perc (mod) ////////////////
	foreach ($rider_uci_points as $k => $rider) {
		$races_mod=0;
		$races_mod=($rider["races_total_perc"]+$rider["sos"]["sos_perc"])/2;
		$races_mod=number_format($races_mod,3);
		$rider_uci_points[$k]["races_mod"]=$races_mod;
	}
	
	//////////////// sort by points and add rank ///////////////////////
	usort($rider_uci_points,'points_sort');
	$counter=0;
	$prev_pts=0;
	foreach ($rider_uci_points as $k=>$rider) {
		$uci_points=$rider["points"];
		if ($uci_points!=$prev_pts) {
			$counter++;
		}
		if ($rider["points"]==0) {
			$rider_uci_points[$k]["uci_points_rank"]="NR";
		} else {
			$rider_uci_points[$k]["uci_points_rank"]=$counter;
		}
		$prev_pts=$uci_points;
	}

	//////////////// sort by races and add rank ///////////////////////
	usort($rider_uci_points,'races_sort');
	$counter=0;
	$prev=0;
	foreach ($rider_uci_points as $k=>$rider) {
	//print_r($rider);
	//	if ($rider["total_races"]==1) {
	//		$rider_uci_points[$k]["race_total_rank"]="n/a"; // we essentially ignore them b/c they have one race //
	//	} else {
			$race_perc=$rider["races_mod"];
			//echo $rider["name"]." - ".$rider["races_mod"]."<br>";
			if ($race_perc!=$prev) {
				$counter++;
			}
			$rider_uci_points[$k]["race_total_rank"]=$counter;
			$prev=$race_perc;	
	//	}	
	}	

	//////////////// sort by winning % and add rank ///////////////////////
	usort($rider_uci_points,'winning_sort');
	$counter=0;
	$prev=0;
	foreach ($rider_uci_points as $k=>$rider) {
		$win_perc=$rider["winning_perc"];
		if ($win_perc!=$prev) {
			$counter++;
		}
		if ($win_perc==0) {
			$rider_uci_points[$k]["winning_perc_rank"]=0;
		} else {
			$rider_uci_points[$k]["winning_perc_rank"]=$counter;
		}
	...
	//	echo $rider["name"]." - ".$rider["winning_perc"]." - ".$rider["winning_perc_rank"]."<br />";
	//}
	...
	foreach ($rider_uci_points as $rider) {
		...
		// calc computer % for computer ranking (no vote) //
		$comp_total=($rider["points_perc"]+$rider["races_mod"]+$winning_perc+$rider["sos"]["sos_perc"])/4;
		$comp_total=number_format($comp_total,3);
		$final_arr["comp_total"]=$comp_total;

		// cacl total //
		//echo $rider["points_perc"];
		$perc_total=($rider["points_perc"]+$rider["races_mod"]+$winning_perc+$rider["sos"]["sos_perc"]+$rider["vote"]["perc"])/5;
		$total=number_format($perc_total,3);
	...
	///// computer sort and rank /////
	usort($final_rider_info,'computer_sort');
	$counter=0;
	$prev_total=0;
	foreach ($final_rider_info as $k=>$rider) {
		if ($rider["comp_total"]!=$prev_total) {
			$counter++;
		}

		//echo $counter." - ".$rider["comp_total"]."<br>";

		$final_rider_info[$k]["comp_rank"]=$counter;
		$prev_total=$rider["comp_total"];	
	}
	...
	//$final_rider_info_64=base64_encode($final_rider_info_s);
	$final_rider_info_64=base64_encode(gzdeflate($final_rider_info_s));

//print_r(unserialize(gzinflate(base64_decode($final_rider_info_64))));

	//echo strlen($final_rider_info_gz)."<br>";
	//echo strlen($final_rider_info_enc)."<br>";
	//echo strlen($final_rider_info_64);
	?>
	<div class="season">
	...
	<form name="add_rank" id="add_rank" action="<?php echo $_SERVER['PHP_SELF']; ?>?page=<?php echo $_GET["page"]; ?>" method="post">
		...
		<input type="hidden" name="data" value='<?php echo $final_rider_info_64; ?>' />
		<input type="hidden" name="add-rank" value="y" />
	</form>
	<div class="math-table">
		<div class="header">
			<div class="col rank">Rank</div>
			<div class="col comp-rank">CPU</div>
		...
		<?php $rank=1; ?>
		<?php foreach ($final_rider_info as $k=>$rider) { ?>
			<?php
			if (isset($rider["vote"]["rank"])) :
				$vote_rank=$rider["vote"]["rank"];
			else :
				$vote_rank=0;
			endif;
			?>
			...
			?>
				<div class="data <?php echo $class; ?>">
					<div class="col rank"><?php echo $rank; ?></div>
					<div class="col comp-rank"><?php echo $rider["comp_rank"]; ?></div>
					<div class="col name"><?php echo $rider["name"]; ?></div>
					<div class="col uci-pts"><?php echo $rider["points"]; ?> (<?php echo $rider["points_perc"]; ?>) (<?php echo $rider["uci_points_rank"]; ?>)</div>
					<div class="col races">
						<!--
						...
						<?php } ?>
						-->
						<!-- Total: --> <?php echo $rider["races_mod"]; ?> (<?php echo $rider["race_total_rank"]; ?>)
					</div>
					<div class="col win-perc"><?php echo $rider["winning_perc"]; ?> (<?php echo $rider["winning_perc_rank"]; ?>)</div>
					<div class="col sos"><?php echo $rider["sos"]["sos_perc"]; ?> (<?php echo $rider["sos"]["sos_rank"]; ?>)</div>
					<div class="col vote"><?php echo $rider["vote"]["perc"]; ?> (<?php echo $rider["vote"]["pts"]; ?>) (<?php echo $vote_rank; ?>)</div>
