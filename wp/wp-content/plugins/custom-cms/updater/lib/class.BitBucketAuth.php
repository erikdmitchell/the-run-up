...
class BitBucketAuth {
	...
	/**
	 * ADDED EM Version 0.1.4
	 *	combining the request api and _request functions to modify it for the zip file, we also mod the curl function
	 */
	public function request_api_zip($http_method='GET', $request_url=false, $params = array(), $local_zip_name='local_zip') {
		if (!$request_url)
			return false;
		...
		$local_zip=plugin_dir_path(dirname(__FILE__)).$local_zip_name.'.zip';
		$local_zip_url=plugin_dir_url(dirname(__FILE__)).$local_zip_name.'.zip';

		// create the zip folder
		$fp = fopen($local_zip,'w');
	
		$ch = curl_init(); // $ch = curl_init($zip_url);
	
		// main curl opts
		curl_setopt($ch, CURLOPT_URL, $url); //  may not need
		curl_setopt($ch, CURLOPT_RETURNTRANSFER,1); // true
		curl_setopt($ch, CURLOPT_SSL_VERIFYHOST,  2); // false
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE); // false  
		curl_setopt($ch, CURLOPT_TIMEOUT,7); // n/a
		//curl_setopt($ch, CURLOPT_AUTOREFERER, 1); // n/a 
		//curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 0); // true
		
		
		// set post and post data if needed ----- n/a
		if (!empty($params) || $http_method == 'POST') {
			curl_setopt($ch, CURLOPT_POST, 1);
			curl_setopt($ch, CURLOPT_POSTFIELDS, $params);
		}
		
		// set custom HTTP METHOD if needed ------ n/a
		if ( $http_method != 'GET' && $http_method != 'POST' ) {
			curl_setopt($ch, CURLOPT_CUSTOMREQUEST, $http_method); 
		}
		
		// helper: print header send to remote server in curl_getinfo
		curl_setopt($ch, CURLINFO_HEADER_OUT, true); // n/a
		
		// set all other custom headers passed with param ------ n/a
		...
		// send request
		$_result = curl_exec ($ch); // $result = curl_exec($ch);	//returns true / false
		$_info = curl_getinfo($ch); // n/a
...
/** EM **/
	 ...
	 */
/*
	public function request_api_2($http_method, $uri, $params = array()){
