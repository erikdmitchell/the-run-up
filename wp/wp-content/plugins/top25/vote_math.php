...
// sort by top25, then remove those outside 25 //
$counter=1;

foreach ($vote_final as $k=>$vote) {
	if ($counter>25) { // 26 makes it some how equal to 25 //
		$rank=0;
		$vote_final[$k]["perc"]=0; // sets perc to zero //
	} else {
		$rank=$counter;
	}
	$vote_final[$k]["rank"]=$rank;
	$counter++;
}
?>